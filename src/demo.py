...
from image_processors.lang_sam_processor import LangSAMProcessor

from utils.utils import get_3d_points
...
parser.add_argument("--connect", help="Sent best postion to Laptop")
...
def demo():
    ...
    # seg_mask, bbox = LangSAMProcessor.detect_obj(cfgs, image)
    # points = get_3d_points(object_handler.cam)
    while True:
        object_handler.receive_input(11)
        object_handler.manipulate()
        
        # object_handler.center_robot(bbox)
        # object_handler.pickup(points, seg_mask, bbox, False)
